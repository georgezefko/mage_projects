...
class NessieCatalog(Catalog):
    ...
    def create_table(
        ...
        print(f"Creating table with identifier: {identifier}")

        iceberg_schema = self._convert_schema_if_needed(schema)
        print(f"Converted schema: {iceberg_schema}")

        location = self._resolve_table_location(location, 'database_name', 'table_name')
        print(f"Resolved table location: {location}")
        ...
        print(f"New table metadata: {metadata}")
        ...
        try:
            ...
            print(f"Metadata written to location: {metadata_location}")
        ...
        print(f"New Iceberg table: {new_iceberg_table}")
        # Get the HEAD reference
        nessie_identifier.resolve(self.nessie)
        assert nessie_identifier.branch_hash is not None, 'No hash'

        print(f"Committing new table to Nessie branch: {nessie_identifier.branch_name}")
        self.nessie.commit(
